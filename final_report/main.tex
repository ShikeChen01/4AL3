...
insightful analysis. Thus, we decided to use predominantly unsupervised learning
...
Predicting stock prices is effectively modeled as a sequential 
decision-making problem rather than a static regression task. Zhang et al. 
[1] argue that Reinforcement Learning (RL) offers a superior framework by 
bypassing explicit forecasting to optimize trade positions directly, thereby 
navigating the low signal-to-noise ratio of financial data. Among these 
methods, Proximal Policy Optimization (PPO) [2] is particularly effective 
due to its ability to stabilize policy updates in non-stationary, 
noisy market environments.

Recent literature highlights the versatility of RL in quantitative finance 
[3]. Huang et al. [4] successfully applied Deep RL to portfolio allocation 
with complex action spaces, while Soleymani and Paquet [5] introduced 
graph-convolutional architectures to capture asset inter-dependencies. 
Furthermore, Jiang et al. [6] proposed model-free frameworks to address 
dynamic market complexities in large-scale settings.

However, a critical challenge lies in defining reward functions that account
 for tail risk. Zakamouline and Koekebakker [7] demonstrate that standard 
 mean-variance metrics fail to capture higher moments like skewness and 
 kurtosis, proposing the Generalized Sharpe Ratio (GSR) as a more robust 
 measure. Complementing this, Lam et al. [8] emphasize the importance of 
 integrating coherent risk measures directly into RL approximation to ensure 
...
data points. \textbf{For the rest of the report, an overall result would refer
...
\begin{itemize}
    ...
    \item Removal of early data points where it is not representative to the current market situation.
    \item Regularization techniques to penalize noisy features.
\end{itemize}
...
The processed data is fed to a reinforcement learning environment to
...
leverage of 2x. However, to adjust such action space to the standard RL, it
...
\begin{itemize}
    ...
    (e.g., Sharpe ratio) and additional risk controls.
    ...
    GRU architectures to capture temporal dependencies.
    \item \textbf{Outcome:} \textit{Partially Followed through.} We successfully implemented an LSTM-based 
    Actor-Critic agent ("Window LSTM"). This aligns with our hypothesis that temporal encoding was 
    necessary for financial time series. However, due to time and complexity constraints, we did not explore
    p-sLSTM or GRU variants. Future work could extend this line of inquiry.

    \item \textbf{Plan: Implement Parallel Rollout Environments.} We intended to accelerate training 
    by parallelizing the environment interaction.
    \item \textbf{Outcome:} \textit{Changed course.} While we identified the Python rollout loop as a 
    bottleneck, implementing true parallel environments with LSTM agents introduced 
    ...
    processing while preserving the temporal integrity required for the LSTM. In theory, due to 
    the special environment, we can theoretically turn everything into matrix operations, but
    due to time constraints, we did not implement this.

    \item \textbf{Plan: Complete Evaluation Pipeline.} We aimed to build a systematic analysis pipeline.
    \item \textbf{Outcome:} \textit{Followed through.} We expanded our evaluation beyond simple MSE 
    to include a robust testing framework that segments performance across "Last 500", 
...
\bibitem{Zhang2020}
Z. Zhang, S. Zohren, and S. Roberts, ``Deep reinforcement learning for trading,'' \textit{The Journal of Financial Data Science}, vol. 2, no. 2, pp. 25--40, 2020.
...
\bibitem{Sun2023}
S. Sun, R. Wang, and B. An, ``Reinforcement learning for quantitative trading,'' \textit{ACM Transactions on Intelligent Systems and Technology}, vol. 14, no. 3, pp. 1--29, 2023.

\bibitem{Huang2020}
G. Huang, X. Zhou, and Q. Song, ``Deep Reinforcement Learning for Portfolio Management,'' \textit{arXiv preprint arXiv:2012.13773}, 2020.

\bibitem{Soleymani2021}
F. Soleymani and E. Paquet, ``Deep Graph Convolutional Reinforcement Learning for Financial Portfolio Management: DeepPocket,'' \textit{arXiv preprint arXiv:2105.08664}, 2021.

\bibitem{Jiang2024}
Y. Jiang, X. He, and W. Zhang, ``Deep Reinforcement Learning for Portfolio Selection,'' \textit{Decision Support Systems}, 2024. [In press].

\bibitem{Zakamouline2009}
V. Zakamouline and S. Koekebakker, ``Portfolio performance evaluation with generalized Sharpe ratios: Beyond the mean and variance,'' \textit{Journal of Banking \& Finance}, vol. 33, no. 7, pp. 1242--1254, 2009.

\bibitem{Lam2023}
T. S. Lam, A. Verma, B. K. H. Low, and P. Jaillet, ``Risk-Aware Reinforcement Learning with Coherent Risk Measures and Non-Linear Function Approximation,'' in \textit{Proc. ICLR}, 2023.
